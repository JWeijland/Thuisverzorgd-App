...
## Voortgangslog
- 2026-08-18: **Huisstijl v4 (Thuisrood) volledig doorgevoerd.** `theme.ts` vervangen door de v4-tokens (Thuisrood merk, Linnenwit achtergrond, Nachtbruine schaduwen, Alarmrood voor fouten), de getekende laag als componenten (`src/ui/getekend/`: kringelstrepen, onderstreping, handcirkel, handpijl, vonkje, golfrand) plus de SVG's in `assets/getekend/`. Paginabalk v4: alle padheaders en GradientHeader op het rode verloop met de officiële golfrand, Bo als wit silhouet-watermerk (BoMono, 12%) en de witte kringelstreep; aanbieder kreeg de donkere Nachtbruin-variant (amber weg), groene en blauwe padbalken weg. Bo is voortaan altijd groen (rol-kleurvarianten verwijderd, brandbookregel). Contrast-sweep: knoppen/iconen met wit op rood naar Gloedrood `primaryMid` (4,9:1), rode tekst op licht naar Diepbaksteen `primaryDark`, StatusPill-info naar Kringblauw, disabled-knoppen warm. Alle v3-hexen (navy's, `D9413A`, `E19E11`, amber) uit de code; iconen/splash/favicon opnieuw (T-logo in rolkleuren op Linnenwit, Bo-mono-rood als favicon); HUISSTIJL.md in de repo, CLAUDE.md en handoff-README bijgewerkt. Lint, typecheck en 89 tests groen. Nog te doen: EAS-update publiceren en nieuwe native build (iconen/splash zijn native).
